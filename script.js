...
if (!reduceMotion && finePointer && 'IntersectionObserver' in window) {
  // cursor-follow liquid refraction + velocity-driven deformation on all glass surfaces
  const liquidState = new WeakMap();
  const glassEls = '.work-row, .tabbar-inner, .hero-chips span, .stack-pill, .contact-list li, .to-top';
  document.querySelectorAll(glassEls).forEach((el) => {
    const tilt = el.classList.contains('work-row');
    el.addEventListener('pointermove', (e) => {
      let s = liquidState.get(el) || { x: 0, y: 0, t: 0 };
      ...
      liquidState.set(el, s);
      const r = el.getBoundingClientRect();
      const cx = (e.clientX - r.left) / r.width - 0.5;
      const cy = (e.clientY - r.top) / r.height - 0.5;
      el.style.setProperty('--gx', `${e.clientX - r.left}px`);
      el.style.setProperty('--gy', `${e.clientY - r.top}px`);
      const shearX = Math.max(-1.6, Math.min(1.6, vx * 0.35));
      const shearY = Math.max(-1.4, Math.min(1.4, -vy * 0.35));
      const bulgeX = Math.max(-0.07, Math.min(0.07, vx * 0.012));
      const bulgeY = Math.max(-0.07, Math.min(0.07, vy * 0.012));
      el.style.transform =
        `translateY(-4px) translate(${(cx * 14).toFixed(1)}px, ${(cy * 11).toFixed(1)}px) ` +
        `scale(${(1 - cy * 0.10 - bulgeX).toFixed(3)}, ${(1 + cy * 0.10 + bulgeY).toFixed(3)}) ` +
        `skewX(${shearX.toFixed(2)}deg) skewY(${shearY.toFixed(2)}deg)` +
        (tilt ? ` rotateX(${(-cy * 3).toFixed(2)}deg) rotateY(${(cx * 3).toFixed(2)}deg)` : '');
    }, { passive: true });
...
const stackInfo = {
  ...
  python: {
    ...
    svg: `<svg viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg">
      ...
      <circle class="sv-eye" cx="140" cy="36" r="4" fill="#050507"/>
    </svg>`,
  },
  flask: {
    name: 'Flask',
